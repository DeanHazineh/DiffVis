...
class ResBlock(nn.Module):
    ...
    def forward(self, x):
        res = self.body(x).mul(
            self.res_scale
        )  # So res_scale is a scaler? just scale all elements in each feature's residual? Why?
        res += x
...
class HDNet(nn.Module):
    def __init__(self, in_ch=28, out_ch=28, conv=default_conv):
        ...
        # define body module
        m_body = [
            ResBlock(conv, n_feats, kernel_size, act=act, res_scale=1)
        ...
        for i in range(1, n_resblocks):
            m_body.append(ResBlock(conv, n_feats, kernel_size, act=act, res_scale=1))
...
class FDL(nn.Module):
    ...
    def tensor2freq(self, x):
        ...
        return torch.real(torch.fft.rfft(y, n=2, norm="forward"))
