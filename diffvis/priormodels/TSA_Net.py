...
def conv_block(
    ...
    if flag_norm:
        return (
            nn.Sequential(conv, norm, activation)
            if flag_norm_act
            else nn.Sequential(conv, activation, norm)
        )
    else:
...
def fully_block(in_dim, out_dim, flag_norm=False, flag_norm_act=True):
    ...
    if flag_norm:
        return (
            nn.Sequential(fc, norm, activation)
            if flag_norm_act
            else nn.Sequential(fc, activation, norm)
        )
    else:
...
class TSA_Transform(nn.Module):
    ...
    def __init__(
        ...
        self.uSpectral = nn.Parameter(
            torch.tensor(float(uAttn[1])), requires_grad=False
        )
        self.mask = (
            nn.Parameter(Spectral_Mask(outChannel), requires_grad=False)
            if flag_mask
            else None
        )
        self.attn_scale = (
            nn.Parameter(torch.tensor(1.1), requires_grad=False) if flag_mask else None
        )
        ...
        self.query_x = Feature_Spatial(
            outChannel, nHead, int(uSpace[1] / 4), uAttn[0], mode
        )
        self.query_y = Feature_Spatial(
            outChannel, nHead, int(uSpace[0] / 4), uAttn[0], mode
        )
        self.query_lambda = Feature_Spectral(outChannel, nHead, spec_dim, uAttn[1])
        self.key_x = Feature_Spatial(
            outChannel, nHead, int(uSpace[1] / 4), uAttn[0], mode
        )
        self.key_y = Feature_Spatial(
            outChannel, nHead, int(uSpace[0] / 4), uAttn[0], mode
        )
    ...
    def forward(self, image):
        ...
        energy_x = torch.bmm(feat_qx, feat_kx.permute(0, 2, 1)) / torch.sqrt(
            self.uSpatial
        )
        energy_y = torch.bmm(feat_qy, feat_ky.permute(0, 2, 1)) / torch.sqrt(
            self.uSpatial
        )
        energy_lambda = torch.bmm(
            feat_qlambda, feat_klambda.permute(0, 2, 1)
        ) / torch.sqrt(self.uSpectral)
        ...
        pro_feat = (
            feat_value if self.down_sample is None else self.down_sample(feat_value)
        )
        batchhead, dim_c, dim_x, dim_y = pro_feat.size()
        attn_x_repeat = (
            attn_x.unsqueeze(dim=1).repeat(1, dim_c, 1, 1).view(-1, dim_x, dim_x)
        )
        attn_y_repeat = (
            attn_y.unsqueeze(dim=1).repeat(1, dim_c, 1, 1).view(-1, dim_y, dim_y)
        )
        pro_feat = pro_feat.view(-1, dim_x, dim_y)
        pro_feat = torch.bmm(pro_feat, attn_y_repeat.permute(0, 2, 1))
        pro_feat = torch.bmm(
            pro_feat.permute(0, 2, 1), attn_x_repeat.permute(0, 2, 1)
        ).permute(0, 2, 1)
        ...
        pro_feat = (
            pro_feat.contiguous().view(batchhead, self.outChannel, -1).permute(0, 2, 1)
        )
        ...
        pro_feat = torch.cat(
            torch.split(pro_feat, int(batchhead / self.nHead), dim=0), dim=1
        ).permute(0, 2, 3, 1)
...
class Feature_Spatial(nn.Module):
    ...
    def __init__(self, inChannel, nHead, shiftDim, outDim, mode):
        ...
        self.conv2 = conv_block(
            nHead, nHead, kernel[mode[1]], stride[mode[1]], padding[mode[1]], _NORM_ATTN
        )
...
class TSA_Net(nn.Module):
    ...
    def __init__(self, in_ch=28, out_ch=28):
        ...
        self.transform1 = TSA_Transform(
            (256, 256), 64, out_ch, 8, (48, 30), [1, 1], True
        )
...
class Encoder_Triblock(nn.Module):
    def __init__(
        self, inChannel, outChannel, flag_res=True, nKernal=3, nPool=2, flag_Pool=True
    ):
        ...
        if flag_res:
            ...
            self.layer2 = conv_block(
                outChannel, outChannel, nKernal, flag_norm=_NORM_BONE
            )
...
class Decoder_Triblock(nn.Module):
    def __init__(
        ...
        if flag_res:
            ...
            self.layer2 = conv_block(
                outChannel * 2, outChannel * 2, nKernal, flag_norm=_NORM_BONE
            )
        self.layer3 = conv_block(
            outChannel * 2, outChannel, nKernal, flag_norm=_NORM_BONE
        )
