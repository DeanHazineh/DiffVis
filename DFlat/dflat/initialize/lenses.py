...
def focusing_lens(
    ...
    phase1 = -2 * np.pi / wavelength_set * (np.sqrt(depth_set**2 + x**2 + y**2))
    phase2 = (
        -2
        * np.pi
        / wavelength_set
    ...
    phase = np.exp(1j * phase1) * np.exp(1j * phase2)
...
#     in_size=[13333 / 2, 13333 / 2],
#     in_dx_m=[300e-9 * 2, 300e-9 * 2],
#     wavelength_set_m=[532e-9],
#     depth_set_m=[1],
#     fshift_set_m=[[0, 0]],
#     out_distance_m=25e-3,
#     aperture_radius_m=None,
#     radial_symmetry=False,
# )
...
# ax[0].imshow(amp[0], vmin=0, vmax=1)
# ax[1].imshow(phase[0], cmap="hsv", vmin=-np.pi, vmax=np.pi)
