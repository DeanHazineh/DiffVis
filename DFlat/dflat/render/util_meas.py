...
def hsi_to_rgb(
    ...
    if tensor_ordering:
        hsi = hsi.transpose(-3, -1).transpose(-3, -2).contiguous()  # ... h w c
    if adjoint:
        assert 3 == hsi.shape[-1], "Channel dimension must be 3 for adjoint transform"
    else:
        assert (
            len(wavelength_set_m) == hsi.shape[-1]
        ), "Channel dimension must be len(wavelengths) for RGB projection"
    ...
    spec = torch.tensor(spec).type_as(hsi)  # [C, 3]

    if adjoint:  # We are doing the adjoint operation
        out = torch.matmul(hsi, spec.T)
    else:
        out = torch.matmul(hsi, spec)

    if demosaic:
        out = bayer_interpolate(bayer_mask(out))
        assert raw is False, "returning raw measurement cannot request demosaic also"
    elif raw:
        out = bayer_mask(out)
